...
const Events = () => {
  ...
  const ongoing = events.filter((e) => e.type === 'ongoing');
  const completed = events.filter((e) => e.type === 'completed' || e.type === 'past');
        ...
        <div className="container mx-auto px-4">
          {loading ? (
            ...
            <>
                  ...
                  {upcoming.map((e) => (
                    ...
                    <Card className="border-border hover:border-primary transition-all hover:shadow-lg hover:scale-[1.02] overflow-hidden cursor-pointer">
                      ...
                      <CardContent className="p-6">
                        ...
                        <p className="text-muted-foreground text-sm mb-4 line-clamp-2">{e.description}</p>
              ...
              {ongoing.length > 0 && (
                <>
                  <h2 className="text-2xl font-heading font-bold uppercase mb-8">Ongoing Events</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
                    {ongoing.map((e) => (
                      <Link to={`/events/${e.id}`} key={e.id}>
                      <Card className="border-border border-primary/50 hover:border-primary transition-all hover:shadow-lg hover:scale-[1.02] overflow-hidden cursor-pointer">
                        {e.image && <img src={e.image} alt={e.title} className="w-full h-40 object-cover" />}
                        <CardContent className="p-6">
                          <Badge className="bg-green-600 text-primary-foreground mb-4">Ongoing</Badge>
                          <h3 className="font-heading font-bold text-lg mb-2">{e.title}</h3>
                          <p className="text-muted-foreground text-sm mb-4 line-clamp-2">{e.description}</p>
                          <div className="space-y-1 text-sm text-muted-foreground">
                            <div className="flex items-center gap-2"><Calendar className="w-4 h-4 text-primary" /> {e.date}</div>
              ...
              {/* Completed Events */}
              <h2 className="text-2xl font-heading font-bold uppercase mb-8">Completed Events</h2>
              {completed.length === 0 ? (
                <p className="text-muted-foreground mb-16">No completed events yet.</p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
                  {completed.map((e) => (
                    <Link to={`/events/${e.id}`} key={e.id}>
                    <Card className="border-border opacity-75 overflow-hidden hover:opacity-100 transition-all cursor-pointer">
                      {e.image && <img src={e.image} alt={e.title} className="w-full h-40 object-cover grayscale" />}
                      <CardContent className="p-6">
              ...
              {cancelled.length > 0 && (
                <>
                  <h2 className="text-2xl font-heading font-bold uppercase mb-8">Cancelled Events</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {cancelled.map((e) => (
                      <Link to={`/events/${e.id}`} key={e.id}>
                      <Card className="border-border opacity-50 overflow-hidden hover:opacity-75 transition-all cursor-pointer">
                        {e.image && <img src={e.image} alt={e.title} className="w-full h-40 object-cover grayscale" />}
                        <CardContent className="p-6">
